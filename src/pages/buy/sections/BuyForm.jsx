...
import { MapPin, Settings2, ChevronRight, Banknote } from "lucide-react";
...
export default function BuyForm({ formData, setFormData, onSubmit }) {
  ...
  const activeTypeKey = formData?.type?.toLowerCase()?.replace(/\s+|\/+/g, "-");

  useEffect(() => {
    if (!formData?.type) return;

    const key = formData.type.toLowerCase().replace(/\s+|\/+/g, "-");

    if (key === notification.lastType) return;
    ...
    // 🔊 PLAY SOUND
    if (audioRef.current) {
      audioRef.current.currentTime = 0; // reset if already played
      audioRef.current.play().catch(() => {
        // ignore autoplay block error (browser restriction)
      });
    }
  ...
  const activeContent = categoryContent[activeTypeKey] || {
    label: "🔍 Smart Search",
    caption: "Find verified properties tailored to your needs.",
    tagline: "Search smarter. Buy better.",
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-2 pb-10">
      {notification.show && (
        <div className="fixed top-20 right-2 z-50 animate-slide-in">
          <div className="bg-emerald-700 text-white px-5 py-3 rounded-xl shadow-2xl border border-orange-400/20 flex items-start gap-3 max-w-xs relative">
            ...
            <button
              ...
              className="absolute top-2 right-2 text-white/70 hover:text-white text-xs font-bold"
            >
              ✕
            </button>
          </div>
      ...
      {/* 🔊 ENABLE SOUND PROMPT */}
      {!isAudioUnlocked && (
        <div className="fixed bottom-4 right-4 bg-black text-white px-4 py-2 rounded-lg text-xs shadow-lg z-50">
          🔊 Click anywhere to enable sound
        </div>
      )}
      <div
        key={activeTypeKey}
        className="mb-8 text-left transition-all duration-300 animate-fade-in px-4"
      >
        {/* TITLE */}
        <h1 className="text-xl lg:text-4xl font-extrabold text-slate-900 tracking-tight leading-tight">
          Buy Your <span className="text-orange-600">Dream Property</span>
        </h1>

        {/* LABEL */}
        {activeContent.label && (
          <div className="mt-3">
            <div className="text-[18px] lg:text-xl font-black uppercase tracking-widest text-blue-800 flex flex-col gap-1">
              {activeContent.label.split("|").map((part, index) => (
        ...
        {activeContent.tagline && (
          <p className="mt-3 text-base lg:text-xl font-bold text-slate-800 flex items-start gap-2">
            ...
            <ul className="space-y-2">
              {activeContent.caption
                ...
                .map((sentence, i) => (
                  <li
                    key={i}
                    className="flex items-start text-sm lg:text-base text-slate-600 font-medium"
                  >
