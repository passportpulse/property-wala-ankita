...
export default function SellerDashboard({ subPath }) {
  ...
  return (
    <DashboardLayout role="seller" userName="Rajesh Kumar">
      {/* Welcome Banner */}
      <div className="relative bg-slate-900 rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-10 lg:p-14 mb-8 overflow-hidden shadow-2xl group">
         <div className="absolute top-0 right-0 w-40 h-40 sm:w-80 sm:h-80 bg-dark-orange/20 rounded-full blur-[60px] sm:blur-[80px] -mr-10 -mt-10 sm:-mr-20 sm:-mt-20 group-hover:bg-dark-orange/30 transition-all duration-1000"></div>
         <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8 sm:gap-12 text-left">
            <div className="max-w-xl">
               <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl bg-white/5 border border-white/10 mb-4 sm:mb-6 backdrop-blur-md">
                  <Sparkles size={14} className="text-dark-orange" />
                  <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] sm:tracking-[0.3em] text-dark-orange leading-none">Market Performance</span>
               </div>
               <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black text-white leading-tight tracking-tighter uppercase mb-4 sm:mb-6">Empire <span className="text-dark-orange">Estate</span></h2>
               <p className="text-slate-400 text-sm sm:text-lg font-medium leading-relaxed">You have <span className="text-white font-black underline underline-offset-8 decoration-4 decoration-dark-orange">12 active listings</span> gaining traction in West Bengal.</p>
            </div>
            <div className="flex flex-row md:flex-row gap-3 sm:gap-4">
               <button className="flex-1 sm:flex-none flex items-center justify-center gap-3 px-6 sm:px-8 py-4 sm:py-5 rounded-2xl bg-dark-orange text-white text-[10px] sm:text-[11px] font-black uppercase tracking-widest hover:scale-105 transition-all shadow-xl shadow-orange-500/20 active:scale-95">
                  <Plus size={18} /> List Property
               </button>
               <button className="h-14 w-14 sm:h-16 sm:w-16 shrink-0 flex items-center justify-center rounded-2xl bg-white/5 border border-white/10 text-white hover:bg-white/10 transition-all active:scale-95">
                  <TrendingUp size={24} />
               </button>
            </div>
         </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
         <MetricCard label="Inquiries" value="482" icon={<Users className="text-orange-400" />} trend="+12%" />
         <MetricCard label="Views" value="2.1K" icon={<Eye className="text-blue-400" />} trend="+8%" />
         <MetricCard label="Reach" value="84%" icon={<Target className="text-green-500" />} trend="High" />
         <MetricCard label="Rank" value="#12" icon={<Star className="text-dark-orange" />} trend="Top" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8 mb-8">
         ...
         <div className="lg:col-span-1 bg-slate-50 rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-10 border border-slate-100 border-dashed text-left">
            <h3 className="text-base sm:text-lg font-black text-slate-900 uppercase tracking-tight mb-6 sm:mb-8">Quick Actions</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
         <div className="lg:col-span-2 bg-white rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-10 border border-slate-100 shadow-xl overflow-hidden text-left">
            <div className="flex items-center justify-between mb-6 sm:mb-8">
               <h3 className="text-lg sm:text-xl font-black text-slate-900 uppercase tracking-tight">Recent Leads</h3>
               <button className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-dark-orange hover:gap-4 transition-all flex items-center gap-2">View All <ArrowRight size={14} /></button>
            </div>
            <div className="space-y-3 sm:space-y-4">
               <LeadItem name="Sandeep Roy" property="Skyline Flat" status="Interested" time="15m" />
               <LeadItem name="Amitava Ghosh" property="Luxury Villa" status="Site Visit" time="2h" active />
               <LeadItem name="Priya Singh" property="Commercial X" status="Offer Sent" time="1d" />
            </div>
         </div>
         <div className="lg:col-span-1 bg-slate-900 rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-10 border border-slate-800 shadow-2xl text-left overflow-hidden relative">
            <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-500/10 rounded-full blur-3xl" />
            <h3 className="text-base sm:text-lg font-black text-white uppercase tracking-tight mb-6 sm:mb-8">Market Insights</h3>
            <div className="space-y-4">
               <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                  <p className="text-[9px] font-black text-emerald-400 uppercase tracking-widest mb-1">Top Area</p>
                  <p className="text-sm font-bold text-white">City Center, Durgapur</p>
               </div>
               <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                  <p className="text-[9px] font-black text-orange-400 uppercase tracking-widest mb-1">Avg Demand</p>
                  <p className="text-sm font-bold text-white">High (8.4/10)</p>
               </div>
               <button className="w-full py-4 rounded-xl bg-white text-slate-900 text-[10px] font-black uppercase tracking-widest hover:bg-dark-orange hover:text-white transition-all">View Reports</button>
            </div>
         </div>
      </div>
    </DashboardLayout>
