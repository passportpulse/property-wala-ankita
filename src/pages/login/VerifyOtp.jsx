...
export default function VerifyOtp() {
  ...
  const testOtp = searchParams.get("testOtp") || "";
  ...
  const handleVerify = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        // Save to sessionStorage/localStorage dynamically!
        sessionStorage.setItem("bhaiya_role", data.role);
        sessionStorage.setItem("bhaiya_user", JSON.stringify(data));
      ...
      <div className="w-full lg:max-w-md">
        ...
        <div className="flex-1 px-6 pt-4 flex flex-col lg:px-0 lg:pt-0">
          ...
          <div className="bg-white rounded-2xl shadow-md lg:shadow-xl p-6 lg:p-8">
            {testOtp && (
              <div className="mb-5 p-3 rounded-xl bg-orange-100/50 border border-orange-200 text-center text-xs font-black text-dark-orange animate-pulse">
                🔑 DEVELOPMENT OTP: {testOtp}
              </div>
            )}
              ...
              <div className="flex items-center justify-between text-[11px] font-bold uppercase tracking-tight pt-2">
                ...
                <button
                  ...
                  className={`transition-colors ${
                    timer === 0 ? "text-dark-orange hover:underline font-extrabold" : "text-slate-300"
                  }`}
                >
                  Resend OTP
                </button>
