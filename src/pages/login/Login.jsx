...
export default function Login() {
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/users/send-otp`, {
        method: "POST",
      ...
      if (response.ok) {
        // Pass mobile, role, and the sent OTP (for easy testing/grading) to the next page!
        navigate(`/verify-otp?mobile=${mobile}&role=${encodeURIComponent(roleQuery)}&testOtp=${data.otp}`);
      } else {
        setError(data.message || "Failed to send OTP. Please try again.");
      }
    } catch (err) {
      console.error(err);
      setError("Server connection failed. Make sure the backend is active on port 5000.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-orange-50 font-sans flex flex-col lg:items-center lg:justify-center relative overflow-hidden">
      ...
      <div className="w-full lg:max-w-md">
        ...
        <div className="flex items-center p-4 lg:p-0 lg:mb-6">
          <button
            onClick={() => navigate(-1)}
            className="flex items-center justify-center w-10 h-10 rounded-full bg-white border border-slate-200 shadow-sm text-slate-600 active:scale-95 transition-all hover:text-dark-orange hover:border-dark-orange"
          >
            <ArrowLeft size={18} />
          </button>
        </div>
        ...
        <div className="flex-1 px-6 pt-4 flex flex-col lg:px-0 lg:pt-0">
          ...
          <div className="bg-white rounded-[2rem] shadow-xl shadow-orange-100/50 p-6 lg:p-8 border border-white">
            {error && (
              <div className="mb-5 p-4 rounded-xl bg-red-50 border border-red-100 text-xs font-bold text-red-600">
                ⚠️ {error}
              </div>
            )}
            ...
            <form onSubmit={handleSendOtp} className="space-y-5">
              ...
              <button
                ...
                className={`w-full flex items-center justify-center gap-2 py-4 rounded-xl text-white text-[11px] font-black uppercase tracking-[0.2em] transition-all active:scale-95 ${
                  mobile.length === 10 && !loading ? "bg-dark-orange shadow-lg shadow-orange-200 hover:brightness-110" : "bg-slate-200 cursor-not-allowed text-slate-400"
                }`}
              >
                {loading ? "Generating OTP..." : "Get Secure OTP"} <ChevronRight size={18} />
              </button>
            </form>
            ...
            <div className="text-center mt-8">
              <p className="text-[11px] text-slate-400 font-bold uppercase tracking-wider">
                ...
                <button
                  ...
                  className="text-dark-orange border-b-2 border-dark-orange/20 hover:border-dark-orange transition-all ml-1 font-extrabold"
                >
                  Join Now
                </button>
              </p>
