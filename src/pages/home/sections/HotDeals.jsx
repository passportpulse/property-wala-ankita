...
import { ChevronRight, Maximize2, MapPin, ArrowRight, Flame } from "lucide-react";
      ...
      <Container>
        ...
        <div className="flex items-center justify-between mb-6 lg:hidden px-2">
          <div>
            <div className="flex items-center gap-1.5 mb-1">
              <Flame size={14} className="text-coral-red fill-coral-red" />
          ...
          <button onClick={() => navigate("/buy")} className="h-10 w-10 rounded-full border border-slate-200 flex items-center justify-center text-slate-900">
            <ArrowRight size={18} />
          </button>
        </div>
        ...
        <div className="lg:grid lg:grid-cols-12 bg-white lg:shadow-[0_40px_100px_-20px_rgba(253,138,107,0.1)] rounded-[2.5rem] overflow-hidden lg:border lg:border-slate-100">
          ...
          <div className="hidden lg:flex lg:col-span-4 p-14 flex-col justify-between border-r border-slate-100">
            <div className="space-y-10">
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-coral-red">Hot Deals</h2>
                </div>
                <h1 className="text-4xl font-black text-slate-900 tracking-tighter leading-tight">
                  High-Value <br />
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-coral-red to-soft-orange">Opportunities</span>
                </h1>
              </div>
                ...
                {deals.map((deal, idx) => (
                  <button
                    ...
                    className={`group flex items-center justify-between p-5 transition-all duration-300 rounded-2xl ${
                      active === idx ? "bg-slate-900 text-white shadow-xl translate-x-2" : "hover:bg-slate-50 text-slate-500"
                    }`}
                  >
                    <div className="flex flex-col items-start">
                      ...
                      <span className="text-sm font-bold tracking-tight uppercase">{deal.title}</span>
                    </div>
            ...
            <button onClick={() => navigate("/buy")} className="cursor-pointer flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.2em] text-slate-900 hover:text-coral-red transition-colors group">
              Explore All Listings <ArrowRight className="w-4 h-4 group-hover:translate-x-2 transition-transform" />
            </button>
          </div>

          {/* RIGHT SHOWCASE (Desktop) / FULL CARD (Mobile) */}
          <div className="lg:col-span-8 relative aspect-[4/5] lg:aspect-auto lg:min-h-[600px] overflow-hidden group">
            ...
            {deals.map((deal, idx) => (
              ...
              >
                <img src={deal.image} alt={deal.title} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105" />
            ...
            <div className="absolute inset-x-0 bottom-0 z-20 p-6 lg:p-14">
              <div className="max-w-xl animate-in fade-in slide-in-from-bottom-8 duration-700">
                ...
                <div className="flex items-center justify-between border-t border-white/10 pt-6 lg:pt-8">
                  <div>
                    <p className="text-white/50 text-[10px] font-black uppercase tracking-widest mb-1">Exclusive Price</p>
                    <p className="text-4xl lg:text-6xl font-black text-white tracking-tighter">{deals[active].price}</p>
                  </div>
                  <button onClick={() => navigate("/buy")} className="bg-white text-slate-900 h-12 lg:h-16 px-6 lg:px-10 rounded-2xl font-black text-[10px] lg:text-xs uppercase tracking-widest hover:bg-coral-red hover:text-white transition-all shadow-xl">
                    Claim Deal
                  </button>
                </div>
              </div>
            ...
            <div className="absolute top-6 inset-x-0 z-30 flex justify-center gap-1.5 lg:hidden px-6">
              {deals.map((_, idx) => (
                <div key={idx} className={`h-1 rounded-full transition-all duration-500 ${active === idx ? "w-8 bg-white" : "w-4 bg-white/30"}`} />
              ))}
            </div>
