import { X, Camera, Video, MapPin, IndianRupee, User, Phone, MessageSquare, CheckCircle2 } from "lucide-react";
import { useState } from "react";
import SellerPolicyModal from "./SellerPolicyModal";
      ...
      <div className="relative bg-white w-full max-w-lg rounded-t-[2.5rem] sm:rounded-[2rem] shadow-2xl flex flex-col max-h-[92vh] sm:max-h-[90vh] overflow-hidden animate-in slide-in-from-bottom duration-300">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 sm:p-8 space-y-6 custom-scrollbar">
          ...
          <div className="space-y-4 pt-0">
            ...
            {/* Agreement Checkbox */}
            <div className="flex items-start gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 group cursor-pointer" onClick={() => setAgreed(!agreed)}>
              <div className={`mt-1 flex-shrink-0 w-5 h-5 rounded border-2 transition-all flex items-center justify-center
                ${agreed ? "bg-dark-orange border-dark-orange" : "bg-white border-slate-300 group-hover:border-dark-orange"}`}>
                {agreed && <CheckCircle2 size={14} className="text-white" />}
              </div>
              <div className="text-[11px] leading-relaxed font-medium text-slate-500">
                I agree to the <button type="button" onClick={(e) => { e.stopPropagation(); setShowPolicy(true); }} className="text-dark-orange font-black underline decoration-orange-300 hover:text-slate-900 transition-colors">Bhaiya Fair-Play Policy</button>. 
                I confirm that all listing details and photos are 100% accurate and I will update the status within 24 hours of closing.
              </div>
            </div>
          </div>
        </form>
        ...
        <div className="p-5 border-t border-slate-100 bg-white">
          <button 
            ...
            disabled={!agreed}
            className={`w-full py-4 rounded-2xl font-black uppercase tracking-[0.2em] shadow-lg transition-all text-sm
              ${agreed 
                ? "bg-dark-orange text-white shadow-orange-200 hover:bg-slate-900 active:scale-95" 
        ...
        <SellerPolicyModal isOpen={showPolicy} onClose={() => setShowPolicy(false)} />
      </div>
    </div>
  );
}
